...
namespace game_frameworks::shader_utils {
        ...
        unsigned int compile(const std::string &src, SHADER_TYPE shaderType) {
            ...
            if (!success) {
                ...
                throw runtime_error(std::string{"Shader::compile::failed: "} + infoLog);
            }
            return shader;
        }
        ...
        unsigned int link(unsigned int &vertexShader, unsigned int &fragmentShader) {
            ...
            if (!success) {
                ...
                throw runtime_error(std::string{"Shader::link::failed: "} + infoLog);
            }
            ...
            if (!success) {
                ...
                throw runtime_error(std::string{"Shader::validate::failed: "} + infoLog);
            }
